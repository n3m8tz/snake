...
function init(canvas, ctx, player1NameInput, player2NameInput) {
    ...
    for (let i = 0; i < 5; i++) {
        let width = Math.floor(Math.random() * 3 + 5) * box; // Random width between 5 and 15 boxes
        let height = Math.floor(Math.random() * 4 + 3) * box; // Random height between 5 and 15 boxes
        let x = Math.floor(Math.random() * (canvas.width / box - width / box)) * box;
        let y = Math.floor(Math.random() * (canvas.height / box - height / box)) * box;
...
function draw(canvas, ctx, player1NameInput, player2NameInput) {
    ...
    for (let i = 0; i < snake1.length; i++) {
        ctx.fillStyle = (i == 0) ? '#FFFF00' : '#FFFFFF'; // Changed to yellow
